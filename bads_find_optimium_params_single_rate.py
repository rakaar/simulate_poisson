...
print(f"  r:     [{lower_bounds[1]:.3f}, {upper_bounds[1]:.3f}]")
print(f"  k:     [{lower_bounds[2]:.0f}, {upper_bounds[2]:.0f}]")
print(f"  theta: [{lower_bounds[3]:.1f}, {upper_bounds[3]:.1f}]")
print(f"\nInitial guess: N={x0[0]:.0f}, r={x0[1]:.3f}, "
        f"k={x0[2]:.1f}, theta={x0[3]:.2f}\n")

# Define objective function with fixed DDM data
def obj_func(x):
    return objective_function_singlerate(x, ddm_rts_decided, n_trials=n_trials_per_eval, seed=seed)
...
txt_filename = f'bads_optimization_results_singlerate_{timestamp}.txt'
pkl_filename = f'bads_optimization_results_singlerate_{timestamp}.pkl'
...
save_bads_results_txt(
    ...
    optimized_params={
        'N': N_opt,
        'r1': r_opt,  # Using same rate for both
        'r2': r_opt,  # Using same rate for both
        'k': k_opt,
        'c': c_opt,
        'theta': x_opt[3],
        'x_opt': x_opt
...
results_dict = {
    ...
    'optimized_params': {
        'N': N_opt,
        'r': r_opt,  # Single rate
        'r1': r_opt,  # Same for both left and right
        'r2': r_opt,  # Same for both left and right
        'k': k_opt,
        'c': c_opt,
        'theta': x_opt[3],
        'x_opt': x_opt
