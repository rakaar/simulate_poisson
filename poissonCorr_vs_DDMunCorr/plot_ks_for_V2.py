# %%
"""
Plot KS-statistic for V2 results (varying c with fixed corr_factor and noise).
Creates:
...
# %%
# Setup parameters matching the V2 simulation
c_array = np.array([0.2, 0.3, 0.5, 0.7])
corr_factor_array = np.array([1.1])
exponential_noise_array = np.array([0])  # 0ms

results_folder = Path('results_V2')
output_folder = Path('figures_V2')
...
# %%
# FIGURE 2: Raw RTD Comparisons (1x4 subplots)
...
for c_idx, c in enumerate(c_array):
    try:
        ...
        # Add ylabel to first subplot, legend to all
        if c_idx == 0:
...
# %%
print("\n" + "="*70)
print("ALL FIGURES CREATED FOR V2 DATA")
print("="*70)
