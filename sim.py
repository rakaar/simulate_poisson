...
R0 = Nr0 / N
left_spk_train, left_times = gen_side_spk_trains(N, T, dt, R0)
right_spk_train, right_times = gen_side_spk_trains(N, T, dt, R0)
...
# cumulative sum over time; start at 0 at t=0
cum_diff = np.concatenate(([0], np.cumsum(diff_counts)))
...
plt.axhline(theta_E,  color='r', ls='--')
plt.axhline(-theta_E,  color='r', ls='--')

plt.title('Cumulative spike difference (Right - Left)')
plt.tight_layout()
plt.savefig('cum_diff_example.png', dpi=150)
plt.close()

# Monte Carlo: run N_sim trials, record crossing times and per-trial cum_diff
...
for s in tqdm(range(N_sim), desc='Simulating', unit='trial'):
    # Re-simulate until a threshold crossing occurs
    while True:
        ...
        cum_diffs[s] = cum
        if pos_idxs.size > 0:
            cross_times_pos[s] = pos_idxs[0] * dt
        if neg_idxs.size > 0:
            cross_times_neg[s] = neg_idxs[0] * dt
        ...
        if pos_idxs.size > 0 and neg_idxs.size > 0:
            ...
            first_is_pos = False
        cross_times[s] = first_idx * dt
        if first_is_pos:
            first_pos_times.append(first_idx * dt)
        else:
            first_neg_times.append(first_idx * dt)
        break
...
if pos_times.size > 0:
    ...
    axes[1].bar(edges[:-1], pos_pdf, width=np.diff(edges), align='edge', color='tab:red', alpha=0.7, label='+θ_E')
if neg_times.size > 0:
    ...
    axes[1].bar(edges[:-1], neg_pdf, width=np.diff(edges), align='edge', color='tab:blue', alpha=0.7, label='-θ_E')
