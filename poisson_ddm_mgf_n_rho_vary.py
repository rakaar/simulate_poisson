...
for N in N_range:
    for theta in theta_range:
        ...
        r_left = r0 * rl
        print('-----------------------------')
        print(f'Running for N={N}, theta={theta}')
        ...
        ddm_acc_theory[(N,theta)] = ddm_acc
        ddm_rt_theory[(N,theta)] = ddm_mean_rt

        poisson_acc_theory[(N,theta)] = poisson_acc
        poisson_rt_theory[(N,theta)] = poisson_mean_rt
...
for N in N_range:
    ...
    valid_theta_ddm = [theta for theta in theta_range if (N,theta) in ddm_acc_theory]
    valid_theta_poisson = [theta for theta in theta_range if (N,theta) in poisson_acc_theory]
    
    # Only plot if we have valid data
    if valid_theta_ddm:
        plt.scatter(valid_theta_ddm, [ddm_acc_theory[(N,theta)] for theta in valid_theta_ddm], label=f'N={N} (DDM)')
...
for N in N_range:
    # Filter out theta values that don't have entries in the dictionaries
    valid_theta_ddm = [theta for theta in theta_range if (N,theta) in ddm_rt_theory]
    valid_theta_poisson = [theta for theta in theta_range if (N,theta) in poisson_rt_theory]
    
    # Only plot if we have valid data
    if valid_theta_ddm:
        plt.scatter(valid_theta_ddm, [ddm_rt_theory[(N,theta)] for theta in valid_theta_ddm], label=f'N={N} (DDM)')
    if valid_theta_poisson:
