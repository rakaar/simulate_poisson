# theta -> k * theta, R -> m*R, L -> n*L, to keep gamma, omega same. The plan is to see if higher theta values cause cont and discrete DDM to convege
...
from scaling_rates_as_per_theta import scale_rates
...
theta_scaling_factors = [1, 5, 9, 15]

results_by_k = {}

for k in theta_scaling_factors:
    print(f'Running simulations for theta scaling factor k={k}')
    theta_k = k * theta_E
    ...
    for ABL in ABL_arr:
        for ILD in ILD_arr:
            ...
            # Scale rates for this theta scaling factor
            right_rate_k, left_rate_k = scale_rates(k, right_rate, left_rate)

            # DDM terms with scaled rates
            mu = right_rate_k - left_rate_k
            sigma_sq = right_rate_k + left_rate_k
            ...
            for s in range(N_sim):
                while True:
                    r_times = poisson_spk_train(right_rate_k, T)
                    l_times = poisson_spk_train(left_rate_k, T)
            ...
            pos_hit_indices = np.argmax(dv_paths >= theta_k, axis=1)
            neg_hit_indices = np.argmax(dv_paths <= -theta_k, axis=1)

            pos_crossed = dv_paths[np.arange(N_sim), pos_hit_indices] >= theta_k
            neg_crossed = dv_paths[np.arange(N_sim), neg_hit_indices] <= -theta_k
...
results_to_save = results_by_k

with open(f'scale_theta_ddm_simulation_results_{param_type}.pkl', 'wb') as f:
    pickle.dump(results_to_save, f)

print(f"Simulations complete. Results saved to scale_theta_ddm_simulation_results_{param_type}.pkl")
